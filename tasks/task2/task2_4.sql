...
-- и страну производства класса автомобиля. Также отсортировать результаты по классу и затем по средней позиции
-- в порядке возрастания.

WITH class_avg AS (
    SELECT c.class AS class, AVG(r.position) AS avg_pos, COUNT(c.name) AS car_count
    FROM Results r
    ...
    HAVING COUNT(c.name) >= 2
     ...
     cars_avg AS (
         ...
         WHERE c.class IN (SELECT class FROM class_avg)
         GROUP BY c.name, c.class, cl.country)

SELECT cr.car_name AS car_name, cr.car_class AS car_class, cr.average_position AS average_position,
       cr.race_count AS race_count, cr.car_country AS car_country
FROM cars_avg cr
         JOIN class_avg cls on cls.class = cr.car_class
